...
workflow.add_edge(START, "dm_router")

workflow.add_conditional_edges("dm_router", lambda s: s.get("next"), 
    {"storyteller": "storyteller", "combat_agent": "combat_agent", "rules_agent": "rules_agent", "npc_actor": "npc_actor", END: END})
...
def rules_next_step(state: GameState):
    ...
    if any(e['status'] == 'ativo' for e in enemies): return "combat_agent"
    return END

workflow.add_conditional_edges("rules_agent", lambda s: "tools" if s["messages"][-1].tool_calls else rules_next_step(s), 
    {"tools": "tools", "combat_agent": "combat_agent", END: END})

workflow.add_conditional_edges("combat_agent", lambda s: "tools" if s["messages"][-1].tool_calls else END, 
    {"tools": "tools", END: END})
